...
class SmoothStep:
    def __init__(self, n, a, delta, mu):
        """
        Compactly supported smoothstep in [a, a+delta]
        shifted around mu for fast analytic integration.
        """
        self.n = n
        self.a = a
        self.delta = delta
        ...
        # precompute polynomial coefficients c_k
        self.c = np.zeros(self.kmax + 1)
        for k in range(self.kmax + 1):
            c_k = 0.
            for j in range(max(0, k - n - 1), n + 1):
                c_kj = (
                    sn.special.comb(n + j, j) *
                    sn.special.comb(2*n + 1, n - j) *
                    sn.special.comb(n + j + 1, k) *
                    np.power(a, n + j + 1 - k) *
                    np.power(delta, -(n + j + 1))
                )
                c_k += c_kj
            if (n + 1 - k) % 2 != 0:
                c_k = -c_k
        ...
        if mu == 0.:
            ...
            for k in range(self.kmax + 1):
                ...
                for j in range(k+1):
                    mu_jmk = mu**(j-k)
                    val += self.c[j] * sn.special.comb(j, k) * ((-mu)**(j-k))
                self.d[k] = val

    def value(self, x):
        """Evaluate smoothstep at x (vectorized)"""
        x = np.asarray(x)
        y = np.zeros_like(x)
        mask_smooth = (x > self.a) & (x < self.a + self.delta)
        mask_one = (x >= self.a + self.delta)
        y[mask_one] = 1.
        if np.any(mask_smooth):
            xs = x[mask_smooth] - self.mu
            # Horner evaluation
            val = np.zeros_like(xs)
            for dk in reversed(self.d):
                val = val * xs + dk
            y[mask_smooth] = val
        mask_neg = y < 0.
        y[mask_neg] = 0.
...
#         """
#         if not hasattr(self, "coeff_dict"):
#             self.coeff_dict = {}

#         if k in self.coeff_dict:
#             return self.coeff_dict[k]
#         elif (k > 2*self.n + 1) or (k < 0):
#             return 0.
#         else:
#             c_k = 0.
#             for j in range(max(0, k - self.n - 1), self.n + 1):
#                 c_kj = (
#                     sn.special.comb(self.n + j,     j,        ) *
#                     sn.special.comb(2*self.n + 1,   self.n - j) *
#                     sn.special.comb(self.n + j + 1, k,        ) *
...
#             if (self.n + 1 - k) % 2 != 0: 
#                 c_k = - c_k
#             self.coeff_dict[k] = c_k
#             return c_k

#     def compute_all_coeffs(self):
...
class TruncatedPowerLaw_AnalyticalLowSmooth(basic_1dimpdf):

    def __init__(self, alpha, a, b, smooth=False, delta=1., n=1):
        """
        Truncated PowerLaw probability distribution 
        with optional low-end polynomial (smoothstep) smoothing
        allowing for analytical normalisation.
        """
        self.alpha = alpha
        self.a = a
        self.b = b
        ...
        if smooth:
            self.smoothstep = SmoothStep(n, a, delta, 0)
        else:
    ...
    def compute_norm(self):
        ...
        !!! Does not handle the case where b < a + delta
        """
        if self.a >= self.b: 
            norm = 0.
            ...
            for k in range(0, self.smoothstep.kmax + 1):
                norm += (
                    self.smoothstep.d[k] * 
    ...
    def _log_pdf(self, x):
        ...
        """
        xp = get_module_array(x)
        log_pdf_not_norm = self.alpha * xp.log(x)
        if self.smoothstep is not None:
            log_pdf_not_norm += xp.log(self.smoothstep.value(x))
        # print("PL", np.where(np.isnan(log_pdf_not_norm)))
        return log_pdf_not_norm - xp.log(self.norm)
...
# class TruncatedGaussian_AnalyticalLowSmooth(basic_1dimpdf):

#     def __init__(self, mu, sigma, a, b, smooth=False, delta=1., n=1):
#         """
...
#             self.tgm = truncated_gaussian_moments(a, a+delta, 2*n+1, mu, sigma)
#         else:
#             self.smoothstep = None
#             self.tgm = truncated_gaussian_moments(a, b, 0, mu, sigma)
#         self.norm = self.compute_norm()
...
#             norm = self.tgm[0]
#         else:
#             norm = truncated_gaussian_moments(
#                 self.a + self.smoothstep.delta,
#                 self.b,
#                 self.smoothstep.n,
#                 self.mu,
#                 self.sigma,
#             )[0]
#             for k in range(0, 2*self.smoothstep.n+1 + 1):
#                 norm += self.smoothstep.coeff(k) * self.tgm[k]
#         return norm
...
#         """
#         xp = get_module_array(x)
#         log_pdf_not_norm = - (1./2.) * xp.power((x - self.mu) / self.sigma, 2.)
#         if self.smoothstep is not None:
#             log_pdf_not_norm += xp.log(self.smoothstep.value(x))
#         return log_pdf_not_norm - xp.log(self.norm)
...
class TruncatedGaussian_AnalyticalLowSmooth(basic_1dimpdf):
    ...
    def __init__(self, mu, sigma, a, b, smooth=False, delta=1., n=1):
        ...
        if smooth:
            self.smoothstep = SmoothStep(n, a, delta, mu)
            za, zd, zb = (a - mu)/sigma, (a+delta - mu)/sigma, (b - mu)/sigma
            self.J =  gaussian_weighted_poly_integral(za, zd, 2*n+1)
            self.J0 = gaussian_weighted_poly_integral(zd, zb, 0)
        
        else:
            self.smoothstep = None
            za, zb = (a - mu)/sigma, (b - mu)/sigma
            self.J = gaussian_weighted_poly_integral(za, zb, 0)
    ...
    def compute_norm(self):
        ...
        if self.a >= self.b: 
            ...
            norm = self.sigma * self.J[0]
        else:
            norm = 0 
            for k in reversed(range(0, self.smoothstep.kmax + 1)):
                print(k)
                norm = norm*self.sigma + self.smoothstep.d[k] * self.J[k]
            norm += self.J0[0]
            norm *= self.sigma
            print("G norm", norm)
        return norm
    ...
    def _log_pdf(self, x):
        ...
        if self.smoothstep is not None:
            log_pdf_not_norm += xp.log(self.smoothstep.value(x))
        # print("G", np.where(np.isnan(log_pdf_not_norm)))
        return log_pdf_not_norm - xp.log(self.norm)
...
# def truncated_gaussian_moments(a, b, kmax, mu, sigma):
#     """
#     Vectorized computation of
#         I_k(a,b) = int_a^b x^k exp(-((x-mu)/sigma)^2/2) dx
#     Returns moments for k = 0,...,kmax.
#     a, b can be scalars or numpy arrays.
#     Output shape: (kmax+1, *broadcast(a,b))
#     """
#     xp = get_module_array(a)
#     xn = get_module_array_scipy(a)
#     a = xp.asarray(a)
#     b = xp.asarray(b)
...
#     shape = (kmax+1,) + xp.broadcast(za,zb).shape
#     J = xp.zeros(shape)

#     # base moments
#     J[0] = xp.sqrt(xp.pi/2) * (xn.special.erf(zb/xp.sqrt(2)) - xn.special.erf(za/xp.sqrt(2)))
...
#         J[n] = za**(n-1)*expa - zb**(n-1)*expb + (n-1)*J[n-2]

#     # convert to moments of x
#     I = xp.zeros_like(J)
...
def gaussian_weighted_poly_integral(z1, z2, kmax):
    """
    Computes all 
    J_k(z1, z2) = int_z1^z2 z^k exp(-z^2/2) dz
    for k = 0,...,kmax
    """
    ...
    # Limit cases
    J[0] = np.sqrt(np.pi / 2) * (
        sn.special.erf(z2 / np.sqrt(2)) - 
        sn.special.erf(z1 / np.sqrt(2))
    )
    if kmax >= 1: J[1] = exp1 - exp2
...
class PowerLaw2Gaussians_AnalyticalNorm(basic_1dimpdf):
    ...
    def __init__(
        ...
        self.alpha = alpha
        self.lambdag = lambdag
        self.lambdaglow = lambdaglow
        self.meanglow = meanglow
        self.sigmaglow = sigmaglow
        self.minglow = minglow
        self.maxglow = maxglow
        self.meanghigh = meanghigh
        self.sigmaghigh = sigmaghigh
        self.minghigh = minghigh
        self.maxghigh = maxghigh
        self.smooth = smooth
        self.smoothstep_order = smoothstep_order
        self.smoothpl = smoothpl
        self.smoothglow = smoothglow
        self.smoothghigh = smoothghigh

        super().__init__(
            minval=min(minpl, minglow, minghigh), 
            maxval=max(maxpl, maxglow, maxghigh)
        )
        self.powerlaw = TruncatedPowerLaw_AnalyticalLowSmooth(
            alpha=alpha,
            a=minpl,
            b=maxpl,
            smooth=smooth,
        ...
        self.gaussian_low = TruncatedGaussian_AnalyticalLowSmooth(
            mu=meanglow,
            sigma=sigmaglow,
            a=minglow,
            b=maxglow,
        ...
        self.gaussian_high = TruncatedGaussian_AnalyticalLowSmooth(
            mu=meanghigh,
            sigma=sigmaghigh,
            a=minghigh,
            b=maxghigh,
    ...
    def _log_pdf(self, x):
        xp = get_module_array(x)
        pl_term    = (
            self.powerlaw._log_pdf(x) +
            xp.log1p(- self.lambdag)
        )
        glow_term  = (
            self.gaussian_low._log_pdf(x) +
            xp.log(self.lambdag) +
            xp.log(self.lambdaglow)
        )
        ghigh_term = (
            self.gaussian_high._log_pdf(x) +
            xp.log(self.lambdag) +
            xp.log1p(- self.lambdaglow)

        )
        res = xp.logaddexp(xp.logaddexp(pl_term, glow_term), ghigh_term)
        return res
