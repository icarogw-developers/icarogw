...
class galaxy_MF(object):
    def __init__(self, band=None,Mmin=None,Mmax=None,Mstar=None,alpha=None,phistar=None, Q=None, P=None, R=None, z0=None,
                 LLstarcut=None):
        '''
        ...
        LLstarcut: float
            L/L* where to cut the Schechter function, must be brighter than faint end
        '''
        # Note, we convert phistar to Gpc-3
        if band is None:
            self.Mmin, self.Mmax, self.Mstar, self.alpha, self.phistar, self.Q, self.P, self.R, self.z0 = Mmin, Mmax, Mstar, alpha, phistar, Q, P, R, z0
        else:
            if (band=='W1-glade+') | (band=='W1-upglade'):
                self.Mmin, self.Mmax,self.Mstar,self.alpha,self.phistar, self.Q, self.P,self.R, self.z0 = -28, -16.6, -24.09, -1.12, 1.45e-2*1e9, 0. , 0., 0., 0.1
            elif band=='bJ-glade+':
                self.Mmin,self.Mmax,self.Mstar,self.alpha,self.phistar, self.Q, self.P,self.R, self.z0 = -22.00, -16.5, -19.66, -1.21, 1.61e-2*1e9, 0., 0., 0., 0.1
            elif band=='K-glade+':
                self.Mmin,self.Mmax,self.Mstar,self.alpha,self.phistar, self.Q, self.P,self.R, self.z0 = -27.0, -19.0, -23.39, -1.09, 1.16e-2*1e9, 0., 0., 0., 0.1
            elif band=='g-upglade':
                self.Mmin,self.Mmax,self.Mstar,self.alpha,self.phistar, self.Q, self.P, self.R,self.z0 = -24.0, -17.0, -20.14, -1.07, 1.62e-2*1e9, 0., 0., 0., 0.1
            elif band=='r-upglade':
                self.Mmin,self.Mmax,self.Mstar,self.alpha,self.phistar, self.Q, self.P,self.R, self.z0 = -24.0, -17.0, -20.64, -1.09, 1.43e-2*1e9, 0., 0., 0., 0.1
            elif band=='W1-upglade':
                self.Mmin,self.Mmax,self.Mstar,self.alpha,self.phistar, self.Q, self.P, self.R,self.z0 = -26.0, -20.0, -23.80, -1.14, 0.86e-2*1e9, 0., 0., 0., 0.1
            elif band=='j-euclid':
                self.Mmin,self.Mmax,self.Mstar,self.alpha,self.phistar, self.Q, self.P,self.R, self.z0 = -24.0, -19, -22.62, -1.27, 4.62e-03*1e9, 0., 0., 0., 0.1
            elif band=='h-euclid':
                self.Mmin,self.Mmax,self.Mstar,self.alpha,self.phistar, self. Q, self.P, self.R, self.z0 = -24.0, -19, -22.98, -1.22, 4.63e-03*1e9, 0., 0., 0., 0.1
            elif band=='y-euclid':
                self.Mmin,self.Mmax,self.Mstar,self.alpha,self.phistar, self.Q, self.P,self.R, self.z0 = -24.0, -19,-22.35, -1.34, 4.39e-03*1e9, 0., 0., 0., 0.1
            elif band=='vis-euclid':
                self.Mmin,self.Mmax,self.Mstar,self.alpha,self.phistar, self.Q, self.P, self.R, self.z0 = -23.41, -18., -21.53, -1.40, 0.36e-02*1e9, 0., 0.,0., 0.1
            elif band=='vis-euclid-evo':
                self.Mmin,self.Mmax,self.Mstar,self.alpha,self.phistar, self.Q, self.P, self.R, self.z0 = -23.21, -18, -21.75, -1.42, 0.37e-02*1e9, -1.14, -1.01, 0.85, 0.45
    ...
    def get_norm(self,z):
        ...
        phistarobs, Mstarobs, alphaobs = self.get_evol_phi_Mstar_alpha(z)
        ...
        return phistarobs*sx.special.gamma(alphaobs+1)*(sx.special.gammainc(alphaobs+1,xmax)-sx.special.gammainc(alphaobs+1,xmin))
        
    def get_evol_phi_Mstar_alpha(self,z):
        '''
        ...
        xp = get_module_array(z)
        return self.phistarobs*xp.power(10.,self.P*(z-self.z0)), self.Mstarobs+self.Q*(z-self.z0), self.alpha+self.R*(z-self.z0)
    ...
    def log_evaluate(self,M,z):
        ...
        ((alphaobs+1)*0.4*(Mstarobs-M))*xp.log(10.)-xp.power(10.,0.4*(Mstarobs-M))
    ...
    def background_effective_galaxy_density(self, Mthr, z, Msat=None):
            ...
            phis, Mstarobs, alphaobs= self.get_evol_phi_Mstar_alpha(z)
            ...
            if Msat is not None:
                ...
                outp=xp.zeros_like(x_sat)
                outp=xp.where(Msat<Mthr, phis * (n_thr+n_sat), phis * n_thr)
            else:
                outp = phis * n_thr
