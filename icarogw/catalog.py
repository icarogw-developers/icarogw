...
def create_pixelated_catalogs(outfolder,nside,groups_dict,fields_to_take=None,batch=100000,nest=False):
    '''
    Divide a galaxy catalog file into pixelated files
...
def calculate_mthr_pixelated_files(outfolder,
    ...
    with h5py.File(os.path.join(outfolder,'pixel_{:d}.hdf5'.format(pixel)),'r+') as cat:
        ...
        if not  subcat.attrs['mthr_calculated']:
            ...
            subcat.attrs['mthr_fixed'] = mthr_fixed
            ...
            subcat.create_dataset('fainter_than_msat',data=cat['catalog'][apparent_magnitude_flag][:]>=msat)
            subcat.attrs['mthr_calculated'] = True
            subcat.attrs['msat'] = msat
    ...
    if isinstance(Nintegration, np.ndarray):
        ...
        with h5py.File(os.path.join(outfolder,'pixel_{:d}.hdf5'.format(pixel)),'r+') as cat:
            ...
            if not  subcat.attrs['z_grid_calculated']:
                ...
                valid_galaxies = (zmax>zmin) & (zmax<cosmo_ref.zmax) & subcat['brigther_than_mthr'][:] & subcat['fainter_than_msat'][:]& subcat['not_NaN_indices'][:]
...
def initialize_icarogw_catalog(outfolder,outfile,grouping):
    ...
    # TO-DO: Optimize this method as for deep galaxy catalogs it is too demanding
        ...
        # Initialize an array with grid points equally distributed. Now we are
        # going to populate it
        z_grid=np.linspace(1e-6,zcut,Nintegration)
        #Array with the resolution required between point i and i+1
        resolution_grid=np.ones_like(z_grid)*(zcut-1e-6)/Nintegration
    ...
    with h5py.File(outfile,'a') as icat:
        if 'z_grid' not in icat[grouping]:
        ...
        if 'mthr_moc_map' not in icat[grouping]: 
        # Saves the mthr map
            icat[grouping].create_dataset('mthr_moc_map',data=moc_mthr_map.data)
            
        if 'uniq_moc_map' not in icat[grouping]: 
...
class  icarogw_catalog(object):
    ...
    def effective_galaxy_number_interpolant(self,z,skypos,cosmology,average=False):
        ...
        z: xp.array
            Redshift array
        skypos: xp.array
            Array containing the healpix indices where to evaluate the interpolant
        cosmology: class
        ...
        # Basically tells that if you are above the maximum interpolation range or below, you detect nothing
        # By definition
        ...
        if average:
            ...
            # If you are outside the redshift interpolation range, the values are replaced later
            bgpart=xp.interp(z,z_grid,self.bg_vals_av,left=self.bg_vals_av[0],right=self.bg_vals_av[-1])

            # the values outside the interpolation range have a background given by the out-of-catalog
            if xp.any(idx_out):
            ...
            gcpart=sx.interpolate.interpn((z_grid,pixel_grid),dNgal_dzdOm_vals,xp.column_stack([z,skypos]),bounds_error=False,
                                fill_value=0.,method='linear') # If a posterior sample falls outside, then you return 0
            # No observed catalog below zsat_eff
            gcpart[mask_no_cat] = 0.0
            
            # The values outside the interpolation range have an out-of-catalog given by the full completeness correction
            if msat is None:
...
#the default k-corrections used in gwcosmo, and they depend on the g-r color and redshift
def calc_kcorr_chil_r(z, gr_color):
    x, y = z, gr_color
    p10, p11, p12, p13, p20, p21, p22, p23, p30, p31, p32, p40, p41, p50 = \
        [1.83285,-2.71446,4.97336,-3.66864,-19.7595,10.5033,18.8196,
         6.07785,33.6059,-120.713,-49.299,144.371,216.453,-295.39]
    return p10*x + p11*x*y + p12*x*y**2 + p13*x*y**3 + p20*x**2 + p21*y*x**2 + p22*(x**2)*(y**2) + p23*(x**2)*(y**3) + \
        p30*x**3 + p31*y*x**3 + p32*(x**3)*(y**2) + p40*x**4 + p41*y*x**4 + p50*x**5
...
class galaxy_catalog(object):
    ...
    def effective_galaxy_number_interpolant(self,z,skypos,cosmology,dl=None,average=False):
        ...
        if isinstance(self.mthr_map_cpu, str):
            return xp.zeros(len(z)).reshape(originshape), (self.sch_fun.background_effective_galaxy_density(-xp.inf*xp.ones_like(z), Msat=None)*cosmology.dVc_by_dzdOmega_at_z(z)).reshape(originshape)
