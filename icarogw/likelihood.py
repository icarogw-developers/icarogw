...
class hierarchical_likelihood_v1_bright(bilby.Likelihood):
    ...
    def __init__(self, list_posterior_samples_dict_bright, list_detection_probabilities,
                 injections, rate_model,
                 nparallel_bright=None, neffPE=20, neffINJ=None, likelihood_variance_thr=None):
        ...
        if likelihood_variance_thr is not None:
            ...
            if neffINJ is None:
                ...
                nev_total = np.sum([self.list_posterior_samples_dict_bright[i].n_ev for i in range(len(self.list_posterior_samples_dict_bright))]) 
    ...
    def log_likelihood(self):
        ...
        neff_PE_ev_total = neff_PE_ev_bright

        if xp.any(neff_PE_ev_total<self.neffPE):
            return -xp.inf
        
        nev_total = xp.sum([self.list_posterior_samples_dict_bright[i].n_ev for i in range(len(self.list_posterior_samples_dict_bright))])
        Ns_array = xp.hstack([self.list_posterior_samples_dict_bright[i].Ns_array for i in range(len(self.list_posterior_samples_dict_bright))])
        ...
        if self.rate_model.scale_free:
            ...
            log_likeli = xp.sum([xp.sum(xp.log(self.list_posterior_samples_dict_bright[i].sum_weights))-
                    self.list_posterior_samples_dict_bright[i].n_ev*xp.log(self.injections.pseudo_rate) 
                    for i in range(len(self.list_posterior_samples_dict_bright))])
            
            # log_likeli = xp.sum([xp.sum(xp.log(self.list_posterior_samples_dict_bright[i].sum_weights))-
            #         self.list_posterior_samples_dict_bright[i].n_ev*xp.log(self.rate_model.cw.cosmology.little_h**3.) 
            #         for i in range(len(self.list_posterior_samples_dict_bright))])
            
        else:
            Nexp=self.injections.expected_number_detections()
            # Log likelihood for  the model, Eq. 1.1 on the document
            log_likeli = -Nexp +xp.sum([self.list_posterior_samples_dict_bright[i].n_ev*xp.log(self.injections.Tobs)+
