...
class hierarchical_likelihood_no_selection_effects(bilby.Likelihood):
    def __init__(self, posterior_samples_dict, rate_model, nparallel=None, neffPE=20):
        '''
        Base class for an hierachical liklihood. It just saves all the input requirements for a general hierarchical analysis
    ...
    def log_likelihood(self):
        ...
        xp = get_module_array(self.posterior_samples_dict.Ns_array)
        ...
        # Combine all the terms
        # Log likelihood for scale free model, Eq. 1.3 on the document
        log_likeli = xp.sum(xp.log(self.posterior_samples_dict.sum_weights))-self.posterior_samples_dict.n_ev
